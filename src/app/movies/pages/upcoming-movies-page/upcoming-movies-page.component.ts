...
export class UpcomingMoviesPageComponent {
  ...
  upcomingRes = resource({
    ...
    loader: async ({ params }) => {
      ...
      return await firstValueFrom(this.mdbService.upcoming(page, 'es-ES', genres, match));
    },
  });

  totalPages = computed(() => this.upcomingRes.value()?.total_pages ?? 1);
